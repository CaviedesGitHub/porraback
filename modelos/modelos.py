...
class Apuesta(db.Model):
    ...
    id_apostador = db.Column(db.Integer, db.ForeignKey('apostador.id'))
...
class Apostador(db.Model):
    ...
    contrasena = db.Column(db.String(50))
    nombre_apostador = db.Column(db.String(128))
    apellido_apostador = db.Column(db.String(128))
    correo = db.Column(db.String(128))
    saldo = db.Column(db.String(50))
    apuestas = db.relationship('Apuesta', cascade='all, delete, delete-orphan')
...
class ApostadorSchema(SQLAlchemyAutoSchema):
    class Meta:
        model = Apostador
        include_relationships = True
        load_instance = True
